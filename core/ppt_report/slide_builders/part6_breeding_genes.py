...
class Part6BreedingGenesBuilder:
    """Part 6: 配种记录与隐性基因"""
    ...
    def build(self, data: dict, versions: list = ['standard', 'enhanced']):
        """构建Part 6"""
        logger.info("=" * 60)
        logger.info("开始构建Part 6: 配种记录与隐性基因")
    ...
    def _build_section_divider(self):
        ...
        subtitle_para.text = "配种记录 隐性基因携带 近交系数"
    ...
    def _build_content_slide_standard(self, data: dict):
        ...
        logger.info("✓ 【标准版】配种记录与隐性基因页创建完成（待添加数据内容）")
    ...
    def _build_content_slide_enhanced(self, data: dict):
        ...
        logger.info("✓ 【高级版】配种记录与隐性基因页创建完成（待添加数据内容）")
