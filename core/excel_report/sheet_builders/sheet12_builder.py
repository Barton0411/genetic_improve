"""
Sheet 12构建器: 备选公牛-隐性基因分析
按公牛分别分析隐性基因纯合风险
"""

from .base_builder import BaseSheetBuilder
from openpyxl.styles import PatternFill, Font, Alignment, Border, Side
from openpyxl.chart import BarChart, Reference
from openpyxl.utils import get_column_letter
import logging

logger = logging.getLogger(__name__)


class Sheet12Builder(BaseSheetBuilder):
    """
    Sheet 12: 备选公牛-隐性基因分析

    包含内容:
    - 每个备选公牛的隐性基因分析表格（垂直排列）
    - 每个表格右侧配分组水平条形图（成母牛/后备牛/全群对比）
    """

    def build(self, data: dict):
        """
        构建Sheet 12: 备选公牛-隐性基因分析

        Args:
            data: 包含备选公牛隐性基因分析数据
                - bulls: 公牛列表（按风险从高到低排序）
        """
        try:
            # 检查数据
            if not data or 'bulls' not in data:
                logger.warning("Sheet12: 缺少数据，跳过生成")
                return

            bulls = data['bulls']
            if not bulls:
                logger.warning("Sheet12: 没有备选公牛数据")
                return

            # 创建Sheet
            self._create_sheet("备选公牛-隐性基因分析")
            logger.info(f"构建Sheet 12: 备选公牛-隐性基因分析（{len(bulls)}头公牛）")

            # 定义颜色
            self.color_header = PatternFill(start_color="2E5C8A", end_color="2E5C8A", fill_type='solid')  # 深蓝
            self.color_total = PatternFill(start_color="FFC000", end_color="FFC000", fill_type='solid')   # 橙色
            self.color_light_gray = PatternFill(start_color="F2F2F2", end_color="F2F2F2", fill_type='solid')

            current_row = 1

            # 为每个公牛创建独立的表格和图表
            for idx, bull in enumerate(bulls, 1):
                logger.info(f"创建公牛{idx}/{len(bulls)}: {bull['bull_id']}")

                # 创建公牛表格
                current_row = self._create_bull_table(bull, current_row)

                # 创建图表（右侧）
                chart_row = current_row - 8  # 图表从表格标题行开始
                self._create_bull_chart(bull, chart_row)

                # 公牛之间空3行
                current_row += 3

            # 设置列宽
            self.ws.column_dimensions['A'].width = 15   # 基因列
            self.ws.column_dimensions['B'].width = 16   # 成母牛-纯合头数
            self.ws.column_dimensions['C'].width = 12   # 占比
            self.ws.column_dimensions['D'].width = 16   # 后备牛-纯合头数
            self.ws.column_dimensions['E'].width = 12   # 占比
            self.ws.column_dimensions['F'].width = 16   # 全群-纯合头数
            self.ws.column_dimensions['G'].width = 12   # 占比

            # 冻结首行
            self._freeze_panes('A2')

            logger.info(f"✓ Sheet 12构建完成")

        except Exception as e:
            logger.error(f"构建Sheet 12失败: {e}", exc_info=True)
            raise

    def _create_bull_table(self, bull: dict, start_row: int) -> int:
        """
        创建单个公牛的隐性基因分析表格

        Args:
            bull: 公牛数据
            start_row: 起始行

        Returns:
            下一个可用行号
        """
        # 公牛标题行
        title_text = f"【公牛{bull['bull_id']}】原始号：{bull['original_bull_id']}"
        cell = self.ws.cell(row=start_row, column=1, value=title_text)
        cell.font = Font(size=12, bold=True, color="FFFFFF")
        cell.fill = self.color_header
        cell.alignment = Alignment(horizontal='left', vertical='center')
        # 合并A-G列
        self.ws.merge_cells(f'A{start_row}:G{start_row}')
        self.ws.row_dimensions[start_row].height = 25
        start_row += 1

        # 在群母牛总数说明行
        summary_text = f"在群母牛总数：成母牛{bull['mature_cow_count']}头 + 后备牛{bull['heifer_count']}头 = 全群{bull['total_cow_count']}头"
        cell = self.ws.cell(row=start_row, column=1, value=summary_text)
        cell.font = Font(size=10, italic=True)
        cell.alignment = Alignment(horizontal='left', vertical='center')
        self.ws.merge_cells(f'A{start_row}:G{start_row}')
        start_row += 1

        # 表头
        headers = [
            '基因',
            f'成母牛-纯合\n(总{bull["mature_cow_count"]}头)',
            '占比',
            f'后备牛-纯合\n(总{bull["heifer_count"]}头)',
            '占比',
            f'全群-纯合\n(总{bull["total_cow_count"]}头)',
            '占比'
        ]

        for col_idx, header in enumerate(headers, 1):
            cell = self.ws.cell(row=start_row, column=col_idx, value=header)
            cell.font = Font(bold=True, color="FFFFFF")
            cell.fill = self.color_header
            cell.alignment = Alignment(horizontal='center', vertical='center', wrap_text=True)
            cell.border = self._get_thin_border()

        self.ws.row_dimensions[start_row].height = 30
        start_row += 1

        # 数据行
        for gene_data in bull['gene_summary']:
            row_data = [
                f"{gene_data['gene_name']} ({gene_data['gene_translation']})" if gene_data['gene_translation'] else gene_data['gene_name'],
                gene_data['mature_homozygous'],
                f"{gene_data['mature_ratio']:.1%}",
                gene_data['heifer_homozygous'],
                f"{gene_data['heifer_ratio']:.1%}",
                gene_data['total_homozygous'],
                f"{gene_data['total_ratio']:.1%}"
            ]

            for col_idx, value in enumerate(row_data, 1):
                cell = self.ws.cell(row=start_row, column=col_idx, value=value)
                if col_idx == 1:
                    cell.alignment = Alignment(horizontal='left', vertical='center')
                else:
                    cell.alignment = Alignment(horizontal='center', vertical='center')
                cell.border = self._get_thin_border()

            start_row += 1

        # 分隔线
        for col_idx in range(1, 8):
            cell = self.ws.cell(row=start_row, column=col_idx)
            cell.border = Border(top=Side(style='medium'))
        start_row += 1

        # 小计行（任意基因纯合）
        total_risk = bull['total_risk']
        row_data = [
            '任意基因纯合小计',
            total_risk['mature_homozygous'],
            f"{total_risk['mature_ratio']:.1%}",
            total_risk['heifer_homozygous'],
            f"{total_risk['heifer_ratio']:.1%}",
            total_risk['total_homozygous'],
            f"{total_risk['total_ratio']:.1%}"
        ]

        for col_idx, value in enumerate(row_data, 1):
            cell = self.ws.cell(row=start_row, column=col_idx, value=value)
            cell.font = Font(bold=True)
            cell.fill = self.color_total
            if col_idx == 1:
                cell.alignment = Alignment(horizontal='left', vertical='center')
            else:
                cell.alignment = Alignment(horizontal='center', vertical='center')
            cell.border = self._get_thin_border()

        start_row += 1

        return start_row

    def _create_bull_chart(self, bull: dict, chart_row: int):
        """
        创建公牛的分组水平条形图（右侧）

        Args:
            bull: 公牛数据
            chart_row: 图表起始行（公牛标题行）
        """
        try:
            # 创建条形图
            chart = BarChart()
            chart.type = "bar"  # 水平条形图
            chart.title = "任意基因纯合风险影响占比"
            chart.style = 10

            # 写入图表数据到隐藏列（从列K开始）
            data_col = 11  # K列
            total_risk = bull['total_risk']

            # 写入分类标签
            self.ws.cell(row=chart_row + 2, column=data_col, value="成母牛")
            self.ws.cell(row=chart_row + 3, column=data_col, value="后备牛")
            self.ws.cell(row=chart_row + 4, column=data_col, value="全群")

            # 写入数据（占比，转换为百分数）
            self.ws.cell(row=chart_row + 2, column=data_col + 1, value=total_risk['mature_ratio'])
            self.ws.cell(row=chart_row + 3, column=data_col + 1, value=total_risk['heifer_ratio'])
            self.ws.cell(row=chart_row + 4, column=data_col + 1, value=total_risk['total_ratio'])

            # 设置数据和分类
            data = Reference(self.ws, min_col=data_col + 1, min_row=chart_row + 2, max_row=chart_row + 4)
            cats = Reference(self.ws, min_col=data_col, min_row=chart_row + 2, max_row=chart_row + 4)

            chart.add_data(data, titles_from_data=False)
            chart.set_categories(cats)

            # 设置图表样式
            chart.height = 6  # 6cm高
            chart.width = 12  # 12cm宽
            chart.x_axis.title = "风险占比"
            chart.y_axis.title = ""
            chart.legend = None  # 不显示图例

            # Y轴设置（百分比格式，0-100%）
            chart.x_axis.scaling.min = 0
            chart.x_axis.scaling.max = 0.15  # 最大15%
            chart.x_axis.numFmt = '0%'

            # 放置图表（在表格右侧，从H列开始）
            anchor_cell = f"H{chart_row}"
            self.ws.add_chart(chart, anchor_cell)

        except Exception as e:
            logger.error(f"创建公牛图表失败: {e}")

    def _get_thin_border(self):
        """获取细边框样式"""
        return Border(
            left=Side(style='thin'),
            right=Side(style='thin'),
            top=Side(style='thin'),
            bottom=Side(style='thin')
        )
