...
class Sheet3TPIDistributionBuilder(BaseSheetBuilder):
    ...
    def build(self, data: dict):
        ...
        try:
            ...
            if present_df is None or present_df.empty:
                logger.warning("Sheet 3-3: 在群母牛TPI分布数据为空，跳过构建")
                return
            ...
            if detail_df is not None and not detail_df.empty:
                # 筛选在群母牛数据（同时筛选性别和是否在场）
                present_detail_df = detail_df[(detail_df['是否在场'] == '是') & (detail_df['sex'] == '母')].copy()
                self._create_normal_distribution_chart(present_detail_df, 'TPI_score', "在群母牛TPI正态分布",
            ...
            if all_df is not None and not all_df.empty:
                ...
                if detail_df is not None and not detail_df.empty:
                    ...
                    self._create_normal_distribution_chart(all_detail_df, 'TPI_score', "全部母牛TPI正态分布",
    ...
    def _create_normal_distribution_chart(self, detail_df, score_column, title, chart_row, chart_col, height_cm, width_cm, data_start_col=52):
        ...
        try:
            ...
            cv = (sigma / mu * 100) if mu != 0 else 0  # 变异系数
            ...
            normal_curve = stats.norm.pdf(x_range, mu, sigma) * n * (bin_edges[1] - bin_edges[0])
            ...
            # 在图表下方添加统计指标表格
            stats_start_row = chart_row + 21  # 图表下方1行
            stats_col = chart_col
