...
def collect_candidate_bulls_detail_data(analysis_folder: Path, cache=None) -> dict:
    """
    收集备选公牛明细数据 (Sheet 14)

    直接读取"备选公牛_近交系数及隐性基因分析结果_*.xlsx"文件
    ...
    try:
        ...
        logger.info(f"成功读取备选公牛明细数据: {len(df)}行, {len(df.columns)}列")
        ...
        logger.error(f"收集备选公牛明细数据时发生错误: {e}", exc_info=True)
        return {}
