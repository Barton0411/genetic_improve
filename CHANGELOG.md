...
## [1.2.0.10] - 2025-10-11 🔧 选配库存检查逻辑优化

### 🔧 问题修复
- **修复选配库存检查逻辑**
  - 之前的bug：当性控冻精全部为0，常规冻精有库存时，系统仍然报错"所有公牛的冻精支数都为0"
  - 根本原因：`_collect_semen_counts()`方法遍历所有标签页时，同一bull_id的支数被后面的标签页覆盖
  - 例如：常规标签页收集到USA123→100支，性控标签页又设置USA123→0支，最终只保留0

### ✨ 优化改进
- **优化_collect_semen_counts()方法**
  - 新增`semen_type`参数支持按类型收集库存
  - `semen_type='常规'`: 只收集常规冻精标签页
  - `semen_type='性控'`: 只收集性控冻精标签页
  - `semen_type=None`: 收集所有标签页并累加同一bull_id的支数
...
- **数据查询**
  - 公牛性状、隐性基因、近交系数查询**不受冻精类型影响**
  - 始终使用原始bull_id查询，同一公牛的遗传数据是固定的
  - 无论常规还是性控冻精，遗传评估结果相同

### 🔄 支持场景
- ✅ 性控冻精=0，常规冻精>0 → 允许选配（使用常规冻精）
- ✅ 常规冻精=0，性控冻精>0 → 允许选配（使用性控冻精）
- ✅ 两种都>0 → 允许选配（根据推荐选择对应类型）
- ❌ 两种都=0 → 报错提示设置冻精支数

### 🧹 代码清理
- 删除重复的`_collect_semen_counts()`方法定义（line 2712-2732）
- 保留改进后的统一方法（line 2873）

---

## [1.2.0.9] - 2025-10-11 🔧 选配冻精预览优化

### 🔧 问题修复
- **修复选配冻精预览去重逻辑**
  - 同一公牛的常规和性控冻精现在可以同时显示
  - 之前的bug：当公牛同时有常规和性控冻精时，只显示一种类型
  - 原因：去重逻辑只按bull_id去重，导致同一公牛的不同冻精类型被过滤

### ✨ 优化改进
- **去重策略优化**
  - 改为按 `(bull_id, semen_type)` 组合去重
  - 同一公牛可以有多条记录（常规一条、性控一条）
  - 每条记录保留独立的支数统计

- **分类逻辑改进**
  - 每条记录根据自己的 semen_type 独立分类
  - 不再使用 bull_id 字典映射（会导致同一公牛只能有一种分类）
  - 逐行处理，支持同一公牛在两个标签页都出现

### 🎯 用户体验改善
- 选配预览标签页
  - "常规冻精"标签：显示所有常规冻精记录
  - "性控冻精"标签：显示所有性控冻精记录
  - 同一公牛既有常规又有性控时，两个标签页都会显示

- 库存显示
  - 常规和性控冻精各自显示独立的支数
  - 例如：USA000000123456 常规30支，性控20支

- 数据库查询不变
  - 继续使用原始 bull_id（无后缀）查询育种数据
  - 隐性基因、近交系数等计算不受影响
