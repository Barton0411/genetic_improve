...
VERSION_HISTORY = [
    {
        ...
        "changes": [
            "🔧 修复选配库存检查逻辑：允许只有一种冻精类型有库存时也可以选配",
            "✨ 优化_collect_semen_counts()方法：支持按冻精类型('常规'/'性控'/None)分别收集库存",
            "🎯 改进库存验证：只要常规或性控任一类型有库存即可选配，两种都为0才报错",
            "📊 业务逻辑优化：性状/隐性基因/近交系数查询不受冻精类型影响，使用原始bull_id",
            "🔄 支持灵活选配场景：性控=0常规>0可选配，常规=0性控>0可选配",
            "🧹 代码清理：删除重复的_collect_semen_counts()方法定义"
        ]
    },
    {
        "version": "1.2.0.9",
        ...
        "author": "Development Team",
        "changes": [
            "🔧 修复选配冻精预览去重逻辑：同一公牛的常规和性控冻精现在可以同时显示",
            "✨ 优化去重策略：改为按(bull_id + semen_type)组合去重，而非仅按bull_id去重",
            "🎯 改进分类逻辑：每条记录根据自己的semen_type独立分类，支持同一公牛多个冻精类型",
            "📊 选配预览优化：同一公牛既有常规冻精又有性控冻精时，两个标签页都会显示对应记录",
            "🔄 支数统计独立：同一公牛的常规和性控冻精各自显示独立的库存支数",
            "🎨 数据库查询不变：继续使用原始bull_id查询育种数据、隐性基因、近交系数（无后缀）"
        ]
    },
    {
        "version": "1.2.0.8",
